...
///
/// The fifth copy of a file that exists in every table's server, and the accepted kind of
/// duplication: five tables loaded into one server process, each with its own log to
/// write through. See `CLAUDE.md`, "Still duplicated, on the server side". Taken from the
/// slot machine's, which is the newest and carries `PutBackWhatLeft`.
///
/// It walks item stacks directly rather than going through `PaymentService`, because
/// both of that service's entry points derive the currency from a trader.
///
...
///   have already gone.
/// - **A debit that half-succeeds is the worst case**, so what left is put back before
///   this reports failure -- see <see cref="PutBackWhatLeft"/>.
/// </summary>
[Injectable]
public class Bank(
    ...
    FarkleLog log) : IBank
{
    ...
    /// <summary>
    /// Takes the stake when a player sits down. Returns false without touching
    /// anything if the player is short -- the caller must not seat a player it
    /// cannot pay out on.
    /// </summary>
    ...
    public void Credit(MongoId sessionId, Wallet wallet, int amount, ItemEventRouterResponse output)
    {
        ...
        // before it is handed over. A win pays two stakes, so this splits when the stake is over half a stack --
        // rarely.
